...
Design backlog and status live in the vault: `~/penny-one/projects/writing/taru/PLAN.md` — its `## Operator directives` section is Hannu's and overrides machine plans; read it before any design work (stub pointer at `./PLAN.md`; an instance without the vault uses `./PLAN.md` itself as the plan). (`CLAUDE.md` is this file's identical twin for Claude Code; keep them in sync.)
...
---
*This file is an identical twin of `CLAUDE.md` (Claude Code reads that one; Codex and other agents read this one). If you change one, change both.*
