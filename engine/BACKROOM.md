...
2. **Fan out.** N candidate setups → `work/NNN-candidates.md` — N and the cycle budget come from `engine/settings.md` (default 4–6). Each carries: the situation; **the question it asks** (never the outcome); NPC intents with their whys; stakes and peril; what it costs. Spread the candidates across emotional keys: at small N no two share a key; at large N no key dominates. If the world keeps an `ideas.md` (the quarry of unplayed material from branches and asides), mine it — an idea spent well retires from the file at the next distill.
